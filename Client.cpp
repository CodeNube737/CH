...
void CClient::connect_socket(std::string addr, int port)
{
  ...
  ipaddr.sin_family = AF_INET; // 7th step
  ipaddr.sin_port = htons(port); // 7th step
  ipaddr.sin_addr.s_addr = inet_addr(addr.c_str()); // 7th step

  connect(_socket, (struct sockaddr*)&ipaddr, sizeof(ipaddr));  // matches as shown in 7th step. connect is for Client... but see also what happens in server
}
...
{
  send(_socket, str.c_str(), str.length(), 0); // 9th step
}
...
bool CClient::rx_str (std::string &str)
{
  ...
  do
  {
    rxbytes = recv(_socket, rxbuff, BUFF_SIZE - 1, 0); // recvfrom... 9th step
...
bool CClient::rx_im(cv::Mat &im)
{
  ...
  do
	{
		rxbytes = recv(_socket, rxbuff, BUFF_SIZE, 0); // 9th step
	} 
  while (rxbytes == -1 && (cv::getTickCount() - start_time) / cv::getTickFrequency() < 1.0);  // Timeout after 1 second
