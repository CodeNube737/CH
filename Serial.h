...
public:
	...
	bool open(std::string commPortName, int bitRate = 115200); // i believ this opens a com port for serial communication
	bool is_open();

  /** Writes a string of bytes to the serial port.
	 *
	 * @param buffer pointer to the buffer containing the bytes (ptr to where the bytes is at)
	 * @param buffLen the number of bytes in the buffer (buffer is a pointer)
	 *
